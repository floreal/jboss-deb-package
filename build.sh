...
sudo rm -Rf $name
mkdir -p $here/src/var/log/jboss $here/tmp
if [ ! -f $archive ]; then
...
find $here/src/var/lib/jboss/server/*/conf -type f -exec sed -i 's/8080/${jboss.http}/g' {} \;
find $here/src/var/lib/jboss/server/*/conf -type f -exec sed -i 's/8443/${jboss.https}/g' {} \;
echo "done"
